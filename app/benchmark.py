...
def main() -> None:
    ...
    print(f"\nClassifying {len(test_assets)} assets with {emb.SCAN_WORKERS} workers...")
    ...
    with ThreadPoolExecutor(max_workers=emb.SCAN_WORKERS) as executor:
        futures = {executor.submit(classify_one, aid, name): (aid, name) for aid, name in test_assets}
        done = 0
        for future in as_completed(futures):
            done += 1
            if done % 50 == 0:
                print(f"  {done}/{len(test_assets)}")
            try:
    ...
    print(f"\n{'Overall':20s} {total_correct:4d}/{total_count:<4d}  {overall:.1f}%")
    if skipped:
        print(f"({skipped} assets skipped: no thumbnail or embedding failed)")
